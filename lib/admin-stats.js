...
import { getDesktopDownloads } from "./desktop-downloads.js";
...
function renderStatsHtml(query = {}) {
    ...
    /* ---- desktop downloads (GitHub Releases, см. lib/desktop-downloads.js) ---- */
    const dl = getDesktopDownloads();
    const dlValue = dl.total === null ? "—" : Number(dl.total).toLocaleString("en-US");
    const dlTitle = dl.total === null
        ? "fetching from github…"
        : `installer ${dl.installer} · portable ${dl.portable} · updated ${fmtDate(dl.fetchedAt)}`;
