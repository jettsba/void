...
function renderStatsHtml(query = {}) {
    ...
    const returnedOnce = stats.visitorsReturnedOnce || 0;
    const newTotal = stats.visitorsNew || 0;
    ...
    const lifeVisitorsTitle =
        ...
        + `return visits ${fmtN(stats.visitorsReturning || 0)} = person-days, not people `
    ...
    const legacyBtn =
        `<a class="seg legacy-toggle${showLegacy ? " is-active" : ""}"
            href="?day=${day}&period=${period}${showLegacy ? "" : "&legacy=1"}"
            title="days before ${STATS_EPOCH_DAY}, when part of the metrics was not collected yet">${showLegacy ? "[x]" : "[ ]"} initial data</a>`;
...
/* Тумблер начальной статистики — визуально отделён от группы периодов
   отступом: это не шестая величина периода, а независимый переключатель. */
.sec-head .controls a.legacy-toggle{margin-left:10px;letter-spacing:.1em}
...
<div class="cols">
    ...
    <div class="col">
        ...
        <div class="readouts">
            ...
            <div class="readout" title="wall-clock time a room had exactly one person — someone waiting or a forgotten tab">
                <div class="readout-label">waiting alone</div>
                <div class="readout-value">${fmtDuration((stats.soloSeconds || 0) + liveSoloSeconds)}</div>
            </div>
...
<div class="chart-row">
    <div class="sec-head">
        ...
        <span class="controls">${periodBtns}${legacyBtn}</span>
    </div>
