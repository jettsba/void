...
function renderStatsHtml(query = {}) {
    ...
    const chartData = chartDays.map(d => {
        ...
        return {
            day: d,
            opens: b ? b.usersRegistered : 0,
            visitors: b ? (b.visitorsUnique || 0) : 0,
...
function renderChart(data) {
    ...
    const maxVal = Math.max(1, ...data.flatMap(d => [d.visitors, d.rooms]));
    ...
    const bars = data.map((d, i) => {
        const cx = x(i);
        const usersH = (d.visitors / niceMax) * innerH;
        const roomsH = (d.rooms / niceMax) * innerH;
        const showLabel = i % labelEvery === 0 || i === data.length - 1;
        const dayShort = d.day.slice(5); // MM-DD
        return `<g>
            <title>${d.day} · visitors: ${d.visitors} · opens: ${d.opens} · rooms: ${d.rooms} · presence: ${fmtDurationShort(d.presence)} · talk: ${fmtDurationShort(d.call)}</title>
