...
export function noteRoomOccupancy(room) {
    if (!room) return;
    const inCall = room.users.size >= 2;

    if (inCall && !room._callStartedAt) {
        room._callStartedAt = Date.now();
    ...
    if (!inCall && room._callStartedAt) {
        closeCallInterval(room, Date.now());
        scheduleStatsWrite();
    }
}
...
/**
 * Дописать незакрытые интервалы разговоров всех живых комнат. Зовётся на
 * shutdown — иначе время активных бесед теряется при каждом деплое.
 */
