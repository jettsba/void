...
import { bumpDaily, scheduleStatsWrite } from "./stats.js";
...
export function mountDownloadBeaconEndpoint(app, jsonMiddleware) {
    app.post("/api/dl-hit", jsonMiddleware, (req, res) => {
        ...
        // Lifetime + дневной срез: без daily не видно динамики («сколько скачали вчера»).
        if (asset === "installer") {
            stats.installerDownloads += 1;
            bumpDaily("installerDownloads");
        } else if (asset === "portable") {
            stats.portableDownloads += 1;
            bumpDaily("portableDownloads");
        } else {
            res.status(204).end();
            return;
        }
