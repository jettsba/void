...
/**
 * Token bucket на сокет. Считать надо по РЕАЛЬНОМУ пику сигналинга, а не «на глаз»:
 * при входе в комнату из N человек клиент поднимает N-1 peer-соединений СРАЗУ, и
 * каждое высыпает свои ICE-кандидаты одним залпом за доли секунды. На Windows с
 * VPN/Hyper-V/VMware-адаптерами host-кандидатов бывает по 8-10 на peer (это видно
 * в failure-логе), плюс srflx и relay — считаем ~12 сообщений на peer.
 *
 *   MAX_ROOM_USERS=10 → 9 peers × 12 ≈ 110 ice + offer/answer ≈ 130 сообщений
 *   в окне ~1 секунды, ещё до того как bucket успеет заметно долиться.
 *
 * Старые 60/30 этот залп НЕ переживали: лишние сообщения дропались молча, а ICE-
 * кандидаты — единственный тип сигналинга без ретрая (offer/answer вытянет
 * sig-stuck watchdog, кандидат потерян навсегда). В failure-логе это выглядело как
 * «remote: none / pairs: none» у одной стороны при нормальных кандидатах у другой.
 * 200/100 даёт ~1.5x запас к худшему легитимному залпу; флуд по-прежнему упирается
 * в потолок, а основная защита от abuse — MAX_CONNECTIONS_PER_IP + payload cap.
 */
export const MSG_BUCKET_CAPACITY = 200;
export const MSG_BUCKET_REFILL_PER_SEC = 100;
...
export function isOriginAllowed(origin) {
    ...
    /* Tauri 2 bundled desktop app шлёт Origin своего кастомного протокола.
       На Windows WebView2 это реальный http://tauri.localhost, на macOS и
       Linux (WKWebView/WebKitGTK) — tauri://localhost. Прежний шаблон вторую
       форму не покрывал, хотя комментарий её и упоминал: сборка под мак
       упёрлась бы в 403 на verifyClient и CORS на /api/* — с диагнозом
       «сервер не пускает», хотя чинится одной строкой здесь.
       Подделать Origin из браузера нельзя (его ставит сам движок), а
       не-браузерный клиент пришлёт любой — так что послабления тут нет. */
    if (/^(https?:\/\/|tauri:\/\/)?tauri\.localhost$/.test(origin)) return true;
